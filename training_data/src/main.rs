use std::{f32::consts::E, fmt, fs, thread};
...
const FILTER_LETTERS_ON_BOARD: usize = 2;
/// Maximum number of words to check before the solver stops trying a given word
const MAXIMUM_WORDS_CHECKED: usize = 100_000;
...
/// Base to use when generating the board; larger numbers favor smaller boards
const BASE: f32 = 2.0; //E;
...
//fn play_from_scratch(letters_array: &[u8], use_long_dictionary: bool, filter_letters_on_board: usize, max_words_to_check: usize, old_board: &[u8], old_min_col: usize, old_max_col: usize, old_min_row: usize, old_max_row: usize) -> JsValue {
fn play_bananagrams(letters: [usize; 26], dict_to_use: &Vec<Word>) -> Option<(Board, usize, usize, usize, usize)> {
    ...
    for (word_num, word) in valid_words_vec.iter().enumerate() {
        ...
        if use_letters.iter().all(|count| *count == 0) {
            return Some((board, min_col, max_col, min_row, max_row));
        }
        else {
            ...
            if let Ok(result) = play_further(&mut board, min_col, max_col, min_row, max_row, &new_valid_words_vec, &valid_words_set, use_letters, 0, &mut words_checked, &mut letters_on_board, FILTER_LETTERS_ON_BOARD, MAXIMUM_WORDS_CHECKED) {
                if result.0 {
                    return Some((board, result.1, result.2, result.3, result.4));
                }
            }
            else if word_num <= 6 {
                words_checked = 0;
...
fn main() {
    let mut dictionary: Vec<Word> = include_str!("../../updated_short_dictionary.txt").lines().map(convert_word_to_array).collect();
    ...
    const NUMBER_OF_BOARDS_TO_GENERATE: usize = 1000;
